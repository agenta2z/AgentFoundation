...
class TestConstructCommand:
    ...
    def test_minimal_command(self, inferencer):
        """Minimal command: acli rovodev run 'prompt' --yolo."""
        cmd = inferencer.construct_command("Hello world")
        assert "/usr/bin/acli rovodev run" in cmd
        assert "--yolo" in cmd
    ...
    def test_site_url(self, inferencer_with_all_options):
        """--site-url flag present when site_url is set."""
        cmd = inferencer_with_all_options.construct_command("Hello")
        assert "--site-url https://mysite.atlassian.net" in cmd
