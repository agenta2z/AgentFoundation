"""Google Gemini models via Atlassian AI Gateway.

Mirror of ``ai_gateway_openai_llm.py`` but for the Google/Gemini vendor route.
Gemini is exposed on the AI Gateway through an OpenAI-compatible chat-completions
endpoint at ``/v1/google/publishers/google/v1/chat/completions`` (so the request
and response schemas are the same as OpenAI's), and through the SDK via
``client.google.v1_chat_completions(...)``.

Supported gateway access modes:
- "direct": shell out to ``atlas slauth token`` CLI, send via httpx (no local
  server needed) — verified working.
- "sdk": use the SDK's typed ``client.google.v1_chat_completions`` with a
  pre-minted SLAuth token (no local slauth server needed).
- "slauth_server": use the SDK with ``SlauthServerAuthFilter`` (needs a local
  ``atlas slauth server``).
- "auto": detect/cascade.

Gemini 3.x are reasoning models (they emit reasoning tokens before the visible
answer). Use ``max_new_tokens`` (mapped to ``max_completion_tokens``) generously.
``reasoning_effort`` (low/medium/high) is forwarded when supplied. Unlike Claude,
Gemini accepts ``temperature=0.0`` together with reasoning, so deterministic runs
are possible.
"""
...
# constant ``AIGatewayEndpoints.Google.V1_CHAT_COMPLETIONS``.
GEMINI_CHAT_COMPLETIONS_PATH = "/v1/google/publishers/google/v1/chat/completions"


class AIGatewayGeminiModels(StrEnum):
    """Google Gemini models exposed via the AI Gateway (google vendor route).

    Only identifiers confirmed available (HTTP 200) for the offline-eval
    use-case are listed. Other ids (e.g. ``gemini-3.1-pro``, ``gemini-3-pro``)
    returned 400 and are intentionally omitted. Add more here as the gateway
...
def _get_messages(
    ...
    """Normalise prompt/messages input into OpenAI-style chat-message dicts."""
...
def _build_request_payload(
    ...
    **kwargs,
    ...
    """Build the Gemini (OpenAI-compatible) chat-completions request payload.

    Gemini uses ``max_completion_tokens`` and accepts ``temperature`` (incl.
    0.0), ``top_p``, ``seed``, and ``reasoning_effort``.
    """
...
def _send_via_sdk(model_str: str, request_payload: dict, config: dict, timeout: float = 120) -> dict:
    ...
    from ai_gateway.models.google.chat_completions import GoogleChatCompletionRequest
    ...
    req = GoogleChatCompletionRequest(**request_payload)
    response = client.google.v1_chat_completions(RequestWrapper(req))
