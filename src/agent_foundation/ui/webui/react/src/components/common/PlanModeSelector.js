        ...
        <Select
          ...
          sx={(muiTheme) => ({
            ...
            '&:hover .MuiOutlinedInput-notchedOutline': {
              borderColor: muiTheme.custom.surfaces.inputBorderHover,
            },
      ...
      <Box sx={{ position: 'relative' }}>
        {isLoading ? (
          ...
          <>
            <ClickToEditMarkdown
              value={currentValue || ''}
              onChange={(newValue) => onChange(variableName, newValue)}
              placeholder={placeholder || 'Customize the research plan here...'}
              minRows={8}
              maxRows={20}
              helperText={isModified ? '✏️ Modified from template - click ↺ to reset' : 'Optional - you can leave this as-is or customize'}
            />
              ...
              <Tooltip title="Reset to original template">
                <IconButton
                  ...
                  sx={(muiTheme) => ({
                    ...
                    zIndex: 10,
                    '&:hover': {
                      backgroundColor: muiTheme.custom.surfaces.overlayActive,
                      color: 'primary.main',
                    },
                  })}
                >
